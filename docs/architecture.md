...
у приложения. Счётчик «Использовали» — уникальные люди с любым действием в боте или кабинете
за день (включая первое `/start`). Кто видит отчёт — allowlist `ADMIN_TELEGRAM_IDS`.
...
```
[🔄 Зафиксировать прибыль] → [Все в обороте]
